...
const TerritoryLeadsAnalytics = () => {
  ...
  // --- Integration of your usePagination Hook ---
  const {
    currentPage,
    setCurrentPage,
    totalPages,
    paginatedData: currentLeads,
  } = usePagination(filteredLeads, 10);

  // Reset page when territory changes
  useEffect(() => {
    setCurrentPage(1);
  }, [territory, setCurrentPage]);
  ...
  const getMergedData = () => {
    ...
    const totalConverted = Object.values(data.conversionPerTerritory).reduce((sum, t) => sum + t.converted, 0);
  ...
  const sourceData = territory === "All Territories" ? merged.sources : data.sourceBreakdownPerTerritory[territory];
  const conversionData = territory === "All Territories" ? merged : data.conversionPerTerritory[territory];
  const revenue = territory === "All Territories" ? merged.totalRevenue : data.revenuePerTerritory[territory];
  const totalLeadsCount = territory === "All Territories" ? merged.totalLeads : data.leadsPerTerritory[territory];
  ...
  const getSortedBarChartData = () => {
    ...
    return {
      labels: statusesArray.map(item => item.name),
      data: statusesArray.map(item => item.count),
      colors: statusesArray.map(item => item.name.toLowerCase() === 'unknown' ? '#FF9500' : (item.isActive ? '#34C759' : '#FF2D55'))
    };
  };
  ...
  const barChartData = {
    ...
    datasets: [{ label: "Leads", data: sortedBarData.data, backgroundColor: sortedBarData.colors, borderRadius: 5 }],
  };
  ...
  const pieChartData = {
    ...
    datasets: [{
      ...
      backgroundColor: ["#FF9500", "#34C759", "#FF2D55", "#AF52DE", "#5AC8FA", "#C69C6D", "#8E8E93"],
      hoverOffset: 4,
    }],
  };
  ...
  function getStatusColor(statusName) {
    ...
    if (statusName?.toLowerCase() === 'unknown') return "bg-yellow-100 text-yellow-700";
    if (statusInfo) return statusInfo.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700";
  ...
  const exportToExcel = () => {
    ...
    saveAs(new Blob([excelBuffer]), `territory_leads_${territory}.xlsx`);
  };

  return (
    <div className="p-4 mx-auto font-[system-ui] bg-gray-50 min-h-screen">
      <div className="flex items-center mb-6">
        <button onClick={() => navigate("/reportpage")} className="text-gray-600 hover:text-gray-900 mr-4 text-2xl p-2 rounded-full hover:bg-gray-200 transition-colors">
          <FaArrowLeft />
        </button>
      ...
      <div className="mb-8 flex justify-center">
        <Listbox value={territory} onChange={setTerritory}>
          <div className="relative w-64">
            ...
            <Listbox.Options className="absolute z-10 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-auto text-sm">
              {allTerritories.map((t) => (
                <Listbox.Option key={t} value={t} className={({ active }) => `cursor-pointer select-none relative py-2 px-4 ${active ? "bg-blue-100 text-blue-700" : "text-gray-800"}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <Card icon={<Users size={18} />} title="Total Leads" value={totalLeadsCount} />
      ...
      <div className="mt-10 bg-white rounded-3xl border border-gray-200 p-6 shadow-xl">
        <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
          ...
          <button onClick={exportToExcel} className="flex items-center px-4 py-2 bg-green-600 text-white rounded-full shadow-md hover:bg-green-700 text-sm font-semibold">
