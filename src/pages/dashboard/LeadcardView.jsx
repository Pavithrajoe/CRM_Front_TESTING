...
// ✅ SAVE CURRENT FILTER STATE (for back navigation)
const filterState = {
  fromDate,
  toDate,
  selectedPotential,
...
useEffect(() => {
  ...
  setToDate(f.toDate || "");
  setSelectedPotential(f.selectedPotential || "");
  setSelectedStatus(f.selectedStatus || "");
  setSelectedSource(f.selectedSource || "");
  setSelectedSubSource(f.selectedSubSource || "");
  setSelectedIndustry(f.selectedIndustry || "");
  setSelectedSubIndustry(f.selectedSubIndustry || "");
  setSelectedService(f.selectedService || "");
  setSelectedSubService(f.selectedSubService || "");
  setSearchTerm(f.searchTerm || "");
  setSelectedFilter(f.selectedFilter || "all");

  setFiltersRestored(true);
}, [location.state, filtersRestored]);
...
const dataToDisplay = useMemo(() => {
  ...
  // ✅ BASE DATA SELECTION (ACCESS ONLY HERE)
  ...
  return data.filter((item) => {
    ...
    if (selectedSubSource) {
      if (item.isub_src_id !== Number(selectedSubSource))
        matchesModalFilters = false;
    }
    ...
    if (selectedSubIndustry) {
      if (item.isubindustry_id !== Number(selectedSubIndustry))
        matchesModalFilters = false;
    }
    ...
    if (selectedSubService) {
      if (item.isubservice_id !== Number(selectedSubService))
        matchesModalFilters = false;
    }
...
useEffect(() => {
  ...
  // Save it so replace() can’t kill it
  if (!restoredBlinkId) {
...
}, [displayedData, restoredBlinkId, location.state]);
...
const filteredSubIndustries = useMemo(() => {
  ...
  return allSubIndustries.filter(
    (si) => si.iindustry_parent === Number(selectedIndustry)
  );
        ...
        <>
          {viewMode === "list" && (
            ...
            <div
              ...
              className={`min-w-[600px] grid gap-4 px-4 py-3 border-t cursor-pointer text-sm text-gray-700 transition-all duration-300
                ${
          ...
          {viewMode === "grid" && (
            ...
            <div
              ...
              className={`
                relative group bg-white rounded-xl shadow-lg p-10 cursor-pointer
