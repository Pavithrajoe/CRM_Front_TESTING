...
export default function RemindersCard({remindersData, loading, error}) {
  ...
  const processedReminders = remindersData.map((reminder) => {
    ...
    const firstLetter = (reminder.cremainder_title || "R").charAt(0).toUpperCase();
    ...
    <div className="bg-white rounded-3xl p-6 w-full ">
      ...
      {processedReminders.length > 0 ? (
        <div className="space-y-4 max-h-[calc(4*4rem+3*1rem)] overflow-y-auto">
          {processedReminders.map(({ id, name, time, remaining, firstLetter }) => (
            <div key={id} className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center bg-gray-50 hover:bg-gray-100 transition p-4 rounded-lg space-y-2 sm:space-y-0" >
              <div className="flex items-center gap-3 w-full sm:w-auto">
                {/* First letter avatar only - no image */}
                <div className="w-10 h-10 rounded-full flex items-center justify-center bg-gradient-to-br from-blue-500 to-purple-600 text-white font-semibold text-sm shadow-sm flex-shrink-0">
                  {firstLetter}
                </div>
                <div className="flex flex-col flex-grow min-w-0">
                  ...
                  <div className="text-sm text-gray-500 flex items-center gap-1 break-words">
                    <span role="img" aria-label="clock"> 🕒  </span> Meeting starts at {time}
                  </div>
                </div>
              </div>
