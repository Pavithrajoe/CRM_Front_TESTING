...
const [blinkLeadId, setBlinkLeadId] = useState(null);
const [restoredBlinkId, setRestoredBlinkId] = useState(null);
...
const [appliedFilters, setAppliedFilters] = useState({
  ...
  subService: "",
   ...
   const getSubIdsFromLead = (item) => ({
  subIndustryId:
    item.lead_sub_industry?.isubindustry_id ??
    item.isubindustry_id ??
    item.sub_industry_id ??
    null,

  subServiceId:
    item.lead_sub_service?.isubservice_id ??
    item.isubservice_id ??
    item.sub_service_id ??
    null,

  subSourceId:
    item.lead_sub_source?.isub_src_id ??
    item.isub_src_id ??
    item.sub_source_id ??
    null,
});
...
const dataToDisplay = useMemo(() => {
  ...
  if (selectedFilter === "assignedToMe") {
    ...
    data = allLeads;
  } else if (selectedFilter === "activeLeads") {
  ...
  // ✅ ONLY APPLIED FILTERS (IMPORTANT)
  const {
    potential,
    status,
    source,
...
console.log("Applied Filters:", appliedFilters);
  ...
  return data.filter((item) => {
    ...
    // ============================
    // APPLIED FILTERS ONLY
    // ============================
    const { subIndustryId, subServiceId, subSourceId } =
      getSubIdsFromLead(item);

    // POTENTIAL
    if (potential) {
      const itemPotential =
        item.lead_potential?.clead_name || item.potentialDisplay;
      if (itemPotential !== potential) matchesModalFilters = false;
    }
    ...
    if (status) {
      ...
      if (itemStatus !== status) matchesModalFilters = false;
    }

    // SOURCE / SUB SOURCE
    if (source && !subSource) {
      if (item.lead_source_id !== Number(source))
        matchesModalFilters = false;
    }

    if (subSource) {
      if (Number(subSourceId) !== Number(subSource))
        matchesModalFilters = false;
    }

    // INDUSTRY / SUB INDUSTRY
    if (industry && !subIndustry) {
      if (item.cindustry_id !== Number(industry))
        matchesModalFilters = false;
    }

    if (subIndustry) {
      if (Number(subIndustryId) !== Number(subIndustry))
        matchesModalFilters = false;
    }

    // SERVICE / SUB SERVICE
    if (service && !subService) {
      if (item.iservice_id !== Number(service))
        matchesModalFilters = false;
    }

    if (subService) {
      if (Number(subServiceId) !== Number(subService))
        matchesModalFilters = false;
    }
    ...
    const isWebsite =
      item.website_lead === true || item.website_lead === 1;
    ...
    if (selectedFilter === "websiteLeads") {
      return (
  ...
  showLostLeads,
  showLostDeals,
  appliedFilters, // ✅ ONLY THIS
  hasAllLeadsAccess,
  hasActiveLeadsAccess,
  hasWebsiteLeadAccess,
]);

useEffect(() => setCurrentPage(1), [selectedSubIndustry]);
useEffect(() => setCurrentPage(1), [selectedSubService]);
useEffect(() => setCurrentPage(1), [selectedSubSource]);
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      ...
      try {
        ...
        setPotentials(await potentialsRes.json());

        // Sources
        const sourcesRes = await fetch(ENDPOINTS.MASTER_SOURCE_GET, { headers });
        setSources(await sourcesRes.json());
// Sub Sources
const subSrcRes = await fetch(ENDPOINTS.MASTER_SUB_SOURCE_GET, { headers });
const subSrcData = await subSrcRes.json();
setSubSources(subSrcData?.data || []);

        // Statuses
        const statusesRes = await fetch(ENDPOINTS.MASTER_STATUS_GET, { headers });
        const statusesData = await statusesRes.json();
        setStatuses(statusesData?.response || []);

        // Industries + Sub Industries
        const industriesRes = await fetch(ENDPOINTS.MASTER_INDUSTRY_GET, { headers });
        const industriesData = await industriesRes.json();
        setIndustries(industriesData?.response?.industry || []);
        setAllSubIndustries(industriesData?.response?.subindustries || []);

        const servicesRes = await fetch(ENDPOINTS.MASTER_SERVICE_GET, { headers });
        const servicesData = await servicesRes.json();
   setServices(
  (servicesData?.data || []).map((s) => ({
    iservice_id: s.serviceId,    
    cservice_name: s.serviceName,  
  }))
);


const subServiceRes = await fetch(ENDPOINTS.SUB_SERVICE, { headers });
const subServiceData = await subServiceRes.json();



setSubServices(subServiceData?.data || []);
...
const filteredSubIndustries = useMemo(() => {
  ...
  return allSubIndustries.filter(
    si => Number(si.iindustry_parent) === Number(selectedIndustry)
  );
}, [allSubIndustries, selectedIndustry]);
const filteredSubServices = useMemo(() => {
  if (!selectedService) return [];
  return subServices.filter(
    ss => Number(ss.iservice_parent) === Number(selectedService)
  );
}, [subServices, selectedService]);
const filteredSubSources = useMemo(() => {
  if (!selectedSource) return [];
  return subSources.filter(
    ss => Number(ss.isrc_id) === Number(selectedSource)
  );
}, [subSources, selectedSource]);
  ...
  const leadId =
    restoredBlinkId ??
    location.state?.lastViewedLeadId;

  if (!leadId) return;

  // Save once so replace() doesn't wipe it
  if (!restoredBlinkId) {
    setRestoredBlinkId(leadId);
  }

  // Ensure lead exists on current page
  const exists = displayedData.some(
    (l) => Number(l.ilead_id) === Number(leadId)
  );
...
}, [displayedData, location.state, restoredBlinkId]);
...
//     location.state?.activeTab || "leads",
//   );
//   // const [selectedFilter, setSelectedFilter] = useState(location.state?.activeTab || 'all');
//   const [fromDate, setFromDate] = useState("");
//   const [toDate, setToDate] = useState("");
...
//     (attr) => attr.attribute_name === "Import" && attr.bactive === true,
//   );
//   const hasAllLeadsAccess = useMemo(
//     () =>
//       user_attributes.some(
//         (attr) => attr.attribute_name === "All Leads" && attr.bactive === true,
//       ),
//     [user_attributes],
//   );
//   const hasActiveLeadsAccess = useMemo(
//     () =>
//       user_attributes.some(
//         (attr) =>
//           attr.attribute_name === "Active Leads" && attr.bactive === true,
//       ),
//     [user_attributes],
//   );
//   const hasWebsiteLeadAccess = useMemo(
//     () =>
//       user_attributes.some(
//         (attr) =>
//           attr.attribute_name === "Website Lead" && attr.bactive === true,
//       ),
//     [user_attributes],
//   );
...
//       data = allLeads;
//     } else if (selectedFilter === "activeLeads") {
//       // CHANGED: from "leads" to "activeLeads"
//       if (!hasActiveLeadsAccess) return [];
//       data = allLeads;
//     } else if (selectedFilter === "websiteLeads") {
//       if (!hasWebsiteLeadAccess) return [];
...
//       if (
//         (selectedFilter === "all" && hasAllLeadsAccess) ||
//         (selectedFilter === "activeLeads" && hasActiveLeadsAccess) ||
//         (selectedFilter === "websiteLeads" && hasWebsiteLeadAccess)
//       ) {
//         if (selectedPotential) {
//           const itemPotential =
...
//         const leadDetails =
//           leadsAssigned.find((lead) => Number(lead.ilead_id) === leadId) || {};
...
//           clead_name: leadDetails.clead_name || entry.clead_name || "No Name",

//           corganization:
//             leadDetails.corganization || entry.corganization || "-",

//           cemail: leadDetails.cemail || entry.cemail || "-",

//           iphone_no: leadDetails.iphone_no || entry.iphone_no || "-",
...
//         `${ENDPOINTS.LEAD}${currentUserId}?page=1&limit=10000`,
//         {
//           method: "GET",
//           headers: {
//             "Content-Type": "application/json",
...
//               <div className="overflow-x-auto rounded-2xl shadow-md border bg-white border-gray-200">
//                 <div
//                   className={`min-w-[600px] grid gap-4 px-4 py-3 bg-gray-50 text-gray-800 text-sm font-medium ${selectedFilter === "assignedToMe" ? "grid-cols-11" : "grid-cols-8"}`}
//                 >
//                   {/* Select All checkbox */}
//                   <div className="inline-flex items-center">
//                     <input
...
//                       key={
//                         item.ilead_id ||
//                         `assigned-${item.cemail}-${item.iphone_no}-${item.dcreate_dt || Date.now()}`
//                       }
...
//                     <div onClick={() => goToDetail(item.ilead_id, displayedData)} >
//                       <div className="flex w-full justify-between items-center space-x-10">
//                         <h3 className="font-bold text-lg text-gray-900 truncate mb-1"> {item.clead_name || "-"} </h3>
//                       </div>
//                       <p className="text-gray-900 text-sm mb-2 truncate font-semibold"> {item.corganization || item.c_organization || "-"} </p>
//                       <div className="text-gray-900 text-sm space-y-1 mb-3 font-medium">
//                         {(item.cemail || item.c_email) && (
//                           <p className="flex items-center">
...
//       )}
//       {showSummary && summaryLeadId && (
//         <LeadSummaryModal leadId={summaryLeadId} onClose={() => setShowSummary(false)} />
//       )}
