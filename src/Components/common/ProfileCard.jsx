...
import LeadmodelSummaryModal from "../../pages/dashboard/LeadViewComponents/LeadSummaryModal.jsx";

import InteriorDesignEditForm from "../../Industries/InteriorDesigning/InteriorDesignEditForm.jsx"
import LinkedLeads from "./ProfileCardComponents/LinkedLead/LinkedLead.jsx";
...
const ProfileCard = ({ settingsData,  isLoadingSettings = false,  leadData,  isDeal,  isLost, setShowSummary }) => {
 ...
 const handleEditLead = async (lead) => {
  ...
  // open whatsapp
  const openWhatsApp = (number, countryCode = "") => {
  if (!number) return;
  ...
  if (countryCode && countryCode.trim() !== "") {
    ...
    finalNumber = cleanCode + cleanNumber;
  }
  ...
  const renderStatusBadge = () => {
    if (isLost) {
      return (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-2 rounded-lg text-sm font-semibold text-center mt-4">  LOST LEAD </div>
      );
    }
    
    if (isDeal) {
      return (
        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-2 rounded-lg text-sm font-semibold text-center mt-4"> CONVERTED TO WON </div>
      );
    }
        ...
        <div className="flex items-center space-x-2 justify-end">
         {linkedCount > 0 && (
            <button
              ...
              className="p-2 rounded-xl bg-blue-900 text-white hover:bg-purple-500 transition-all duration-300 transform hover:scale-110 shadow-md focus:outline-none focus:ring-2 focus:ring-purple-300"
              aria-label="Linked Leads"
              title="Linked Leads"
            >
              <FaLink size={18} />
            </button>
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              setSummaryLeadId(profile?.ilead_id);
