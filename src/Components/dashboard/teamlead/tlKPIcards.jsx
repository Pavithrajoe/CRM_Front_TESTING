...
export default function KPIStats() {  
    ...
    const fetchKPIStats = async () => {
      ...
      try {
        const response = await fetch(
          `${ENDPOINTS.LEADS_COUNT_BY_USER_ID}${currentUserId}`,
          {
        ...
        // ✅ If 500 or API failure → reset to 0 instead of error
        if (!response.ok || jsonRes.success === false) {
          console.warn("No leads found for this user. Showing default 0.");
          resetToZero();
          return;
        }
        ...
        // ✅ Any error → reset to zero
        console.warn("API failed. Setting all KPI values to 0.", e);
        resetToZero();
      } finally {
        setLoading(false);
      }
  ...
  const coreKpiData = [
    { title: "Total Leads", value: totalLeadCount, clickHandler: () => navigate("/leadcardview", { state: { activeTab: "all" } }) },
    { title: "Active Leads", value: activeCount, clickHandler: () => navigate("/leadcardview", { state: { activeTab: "leads" } }) },
    { title: "Website Leads", value: websiteLeadCount, clickHandler: () => navigate("/leadcardview", { state: { activeTab: "websiteLeads" } }) },
    { title: "Total Lost", value: lostCount, clickHandler: () => navigate("/leadcardview", { state: { activeTab: "lost" } }) },
  ];
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 grid-rows-2 gap-6">
          {secondRowKpiData.map((kpi, index) => (
            <div
              key={index}
              className="relative min-h-[160px] backdrop-blur-md bg-white/60 border border-white/30 shadow-[0_4px_30px_rgba(0,0,0,0.05)] rounded-2xl p-5 overflow-hidden"
              style={{
