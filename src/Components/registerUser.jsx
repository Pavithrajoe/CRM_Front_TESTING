...
export default function CreateUserForm({ onClose }) {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setErrors((prevErrors) => ({
        ...prevErrors,
        fetchUsers: error.message || 'Failed to load users. Please try again later.',
      }));
    } finally {
  ...
  const fetchRoles = async () => {
    ...
    try {
      ...
      setErrors((prevErrors) => ({
        ...prevErrors,
        fetchRoles: error.message || 'Failed to load roles.',
      }));
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      iCompany_id: 5,
      irole_id: roleOptions.find(role => role.cRole_name === formData.role)?.irole_id || 1,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.details?.toLowerCase().includes('unique constraint failed on the fields: (`cemail`)')) {
  ...
  return (
    <div className="relative inset-0 flex justify-center items-center pt-10 overflow-y-auto z-5">
      <form onSubmit={handleSubmit} className="form-container w-full max-w-2xl mx-auto bg-white shadow-lg rounded-2xl p-4 md:p-6">
        <button type="button" onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-black" aria-label="Close form">
          <X size={20} />
        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input name="employeeName" value={formData.employeeName} onChange={handleChange} placeholder="Enter Employee name" className="w-full border border-gray-300 rounded-xl p-2" />
            ...
            <div>
              ...
              <input name="email" value={formData.email} onChange={handleChange} placeholder="Enter the E-mail address" className="w-full border border-gray-300 rounded-xl p-2" />
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} placeholder="Enter Password" className="w-full border border-gray-300 rounded-xl p-2 text-sm pr-10" />
          ...
          <div className="space-y-4">
            <div>
              ...
              <input name="jobTitle" value={formData.jobTitle} readOnly className="w-full border border-gray-300 rounded-xl p-2 bg-gray-100" />
              {errors.jobTitle && <p className="text-red-500 text-sm">{errors.jobTitle}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium">Reports To *</label>
              <select name="reportsTo" value={formData.reportsTo} onChange={handleChange} className="w-full border border-gray-300 rounded-xl p-2" required>
                <option value="">-- Select a Manager --</option>
                {isUserLoading && <option disabled>Loading managers...</option>}
                {userOptions .filter(user => user.bactive === true || user.bactive === "true")
              ...
              {errors.reportsTo && <p className="text-red-500 text-sm">{errors.reportsTo}</p>}
            </div>
            ...
            <div>
              ...
              <select
                ...
                {isRoleLoading && <option disabled>Loading roles...</option>}
                {roleOptions
                    .filter(role => role.cRole_name.toLowerCase() !== 'reseller') // line for remove reseller role
                    .sort((a, b) => a.cRole_name.localeCompare(b.cRole_name))
                    .map(role => (
        ...
        <div className="flex justify-center mt-8">
          <button type="submit" className="px-6 py-2 bg-black text-white rounded-xl hover:bg-gray-800 transition" disabled={isSubmitting}>
