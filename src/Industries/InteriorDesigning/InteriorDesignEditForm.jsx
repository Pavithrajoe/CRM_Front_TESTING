...
import { ENDPOINTS } from "../../api/constraints.js";  // ✅ CORRECT
...
const InteriorDesignEditForm = ({ profile, onClose, onSave, isReadOnly }) => {
...
iservice_id: Array.isArray(profile?.services)
  ? profile.services.map(s => s.serviceId)
  : profile?.iservice_id
  ? [profile.iservice_id]
  : [],
...
// sub source
  ...
  setSearchService(
   profile.services.map(s => s.serviceName).join(", ")
...
const handleServiceSelect = (serviceId, serviceName) => {
  ...
  setForm(prev => {
    const current = Array.isArray(prev.iservice_id) ? prev.iservice_id : [];

    if (Number(prev.cindustry_id) === 5) {
      const newServices = current.includes(serviceId)
        ? current.filter(id => id !== serviceId)
        : [...current, serviceId];
      
      // 🔥 FIX: Update searchService IMMEDIATELY
      const selectedServices = service.filter(s => newServices.includes(s.serviceId));
      setSearchService(selectedServices.map(s => s.serviceName).join(", "));
      
      return { ...prev, iservice_id: newServices };
    }

    setSearchService(serviceName);
    return { ...prev, iservice_id: [serviceId], isubservice_id: "" };
  });
...
const filteredSubServices = subService.filter(
  sub => Array.isArray(form.iservice_id)
    && form.iservice_id.includes(sub.service_parent)

);

 useEffect(() => {
    if (Number(form.cindustry_id) === 5 && Array.isArray(form.iservice_id)) {
      const selectedServices = service.filter(s => form.iservice_id.includes(s.serviceId));
      setSearchService(selectedServices.map(s => s.serviceName).join(", "));
    }
  }, [form.iservice_id, form.cindustry_id, service]);
  useEffect(() => {
...
const validateForm = () => {
  ...
  if (!form.iservice_id || form.iservice_id.length === 0) {
...
const updateLeadProfile = async () => {
  try {
    ...
    // }
if (Number(form.cindustry_id) === 5) {
  payload.iservice_id = form.iservice_id.map(id => parseInt(id, 10));
}
  ...
  payload.quantity = form.quantity || null;
}
    // if (form.ilead_status_id) {
...
const handleSubmit = async (e) => {
  ...
  <input
    type="text"
    value={
      Number(form.cindustry_id) === 5
        ? service
            .filter(s => Array.isArray(form.iservice_id) && form.iservice_id.includes(s.serviceId))
            .map(s => s.serviceName)
            .join(", ")
        : searchService
    }
  ...
  {isServiceDropdownOpen && service.length > 0 && (
    <div className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-blue-500 ring-opacity-50 overflow-auto focus:outline-none sm:text-sm">
    {service.map(item => {  // 🔥 LINE 1: service.map()
  const selected = Array.isArray(form.iservice_id) && form.iservice_id.includes(item.serviceId);
  return (
    <div
      key={item.serviceId}
      onMouseDown={(e) => e.preventDefault()}
      onClick={() => handleServiceSelect(item.serviceId, item.serviceName)}
...
export default InteriorDesignEditForm
