        ...
        <div className="p-4 sm:p-6 bg-gray-50 min-h-full">
            <div className="bg-white p-4 sm:p-6 rounded-xl shadow-2xl max-w-full lg:max-w-4xl mx-auto max-h-[95vh] overflow-y-auto">
                ...
                <div className="mt-8 text-center pt-6">
                    ...
                    >
                        <FaSave className="inline mr-2" /> Save & Finalize Deal
                    </button>
...
// /**
//  * Generates an initial milestone row structure.
//  * @param {number} sNo - Serial number for the milestone.
//  * @param {number} defaultAmount - The initial amount for this milestone.
//  * @returns {object} The new milestone row object.
//  */
// const initialMilestoneRow = (sNo, defaultAmount) => {
//     return {
//         id: Date.now() + Math.random(), // Unique ID for key/deletion
//         sNo,
//         milestone: `Milestone ${sNo}`,
//         milestoneDate: '',
//         amount: parseFloat(defaultAmount.toFixed(2)),
...
// /**
//  * Calculates a balanced, equal split of an amount across a number of phases.
//  * @param {number} totalAmount - The total amount to be split.
//  * @param {number} totalPhases - The number of phases to split into.
//  * @returns {number[]} An array of amounts for each phase.
//  */
// const calculateBalancedSplit = (totalAmount, totalPhases) => {
//     if (totalPhases <= 0 || isNaN(totalAmount) || totalAmount <= 0) return [];
    
//     const totalRemaining = Math.round(totalAmount * 100); // Work in cents to avoid float issues
//     const baseAmountCents = Math.floor(totalRemaining / totalPhases);
//     let remainderCents = totalRemaining % totalPhases;
...
// const MilestoneDetails = ({ serviceData, onBack, totalBalance }) => {
//     // Convert totalBalance to a number immediately
//     const totalAmount = parseFloat(totalBalance) || 0; 
    
//     // State for Payment Terms and Conditions
//     const [termsAndConditions, setTermsAndConditions] = useState('');
//     // State for Payment Phases (Dropdown value: 0, 2, 3, or 4)
//     const [paymentPhases, setPaymentPhases] = useState(0); 
//     // State for the dynamically generated milestone table data
//     const [milestones, setMilestones] = useState([]);
//     // State to track if the milestones have been initially set (to prevent re-initialization on amount change)
//     const [isInitialized, setIsInitialized] = useState(false);

//     // --- Effect for Initial Auto-Generation based on Phase Selection ---
//     useEffect(() => {
//         const newPhases = parseInt(paymentPhases);
        
//         // Only run if a valid phase is selected AND we haven't initialized yet
//         if (newPhases > 0 && !isInitialized) {
//             const splitAmounts = calculateBalancedSplit(totalAmount, newPhases);
...
//             setIsInitialized(false);
//         }
//     }, [paymentPhases, totalAmount]); // Re-initialize only if paymentPhases changes AND it hasn't been done for this phase count
...
//         const value = parseInt(e.target.value);
//         setPaymentPhases(value);
//         setIsInitialized(false); // Reset initialization flag so the useEffect runs
//     };
...
//         setMilestones(prevMilestones => {
//             const nextSNo = prevMilestones.length > 0 ? Math.max(...prevMilestones.map(m => m.sNo)) + 1 : 1;
//             const newRow = initialMilestoneRow(nextSNo, 0); // Start new rows at $0.00
//             return [...prevMilestones, newRow];
//         });
//     };
...
//     const handleDeleteMilestone = (id) => {
//         setMilestones(prevMilestones => {
//             // Filter out the deleted row and re-sequence S.No.
//             const filtered = prevMilestones.filter(m => m.id !== id);
//             return filtered.map((m, index) => ({
//                 ...m,
//                 sNo: index + 1 // Re-sequence S.No.
//             }));
//         });
//     };
...
//     const handleSave = () => {
//         // --- Validation ---
//         const sumOfMilestones = milestones.reduce((sum, m) => sum + parseFloat(m.amount), 0);
//         const tolerance = 0.01; 
...
//         if (Math.abs(sumOfMilestones - totalAmount) > tolerance) {
//             alert(`Error: Sum of milestone amounts ($${sumOfMilestones.toFixed(2)}) must equal the Total Balance ($${totalAmount.toFixed(2)}). Please adjust.`);
//             return;
//         }
...
//         // --- Final Submission Data Compilation ---
//         const finalSubmission = {
//             ...serviceData, // Data from Step 1
//             termsAndConditions,
//             paymentPhases: paymentPhases,
//             milestones: milestones.map(m => ({
//                 sNo: m.sNo,
//                 milestone: m.milestone,
//                 milestoneDate: m.milestoneDate,
//                 // Ensure amount is correctly rounded to 2 decimal places before submission
//                 amount: parseFloat(m.amount.toFixed(2)), 
//             })),
//             finalTotalBalance: totalAmount.toFixed(2),
//         };
...
//         alert("Data successfully compiled and ready for API submission! Check console for full object.");
//         // ** In a real app, you would now make your final API call here **
//     };

//     // Calculate sum and difference for display
//     const currentMilestoneSum = milestones.reduce((sum, m) => sum + parseFloat(m.amount), 0);
//     const amountDifference = (totalAmount - currentMilestoneSum).toFixed(2);
//     const isSumValid = Math.abs(amountDifference) < 0.01;
...
//             <div className="bg-white p-4 sm:p-6 rounded-xl shadow-2xl max-w-full lg:max-w-4xl mx-auto max-h-[80vh] overflow-y-auto">
...
//                         <FaArrowLeft className="mr-2" /> Back to Service Details
//                     </button>
//                     <h2 className="text-xl sm:text-2xl font-bold text-blue-700">Step 2: Payment Details</h2>
//                 </div>
...
//                         className="w-full border px-3 py-2 rounded-lg text-sm focus:ring-blue-400 focus:border-blue-400 resize-none"
//                     />
//                     <p className="text-xs text-gray-500 text-right">{termsAndConditions.length} / 1000 characters (approx. 200 words)</p>
//                 </div>
...
//                     <div className="mb-8">
//                         <h3 className="text-lg font-semibold mb-3 text-blue-600">
//                             Payment Milestones ({milestones.length} of {paymentPhases} Initial Phases)
//                         </h3>
...
//                         type="button"
//                         onClick={handleSave}
//                         // Disable if no phases selected, no milestones, or sum is invalid
//                         disabled={paymentPhases === 0 || milestones.length === 0 || !isSumValid}
//                         className="w-full sm:w-auto bg-green-600 text-white py-3 px-8 rounded-xl hover:bg-green-800 transition text-lg font-semibold shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
//                     >
//                         <FaSave className="inline mr-2" /> Save & Finalize Deal
//                     </button>
...
//                         </p>
//                     )}
//                     {(paymentPhases === 0 && milestones.length === 0) && (
//                         <p className="mt-3 text-sm text-orange-500 font-medium">
//                             Please select a number of Payment Phases to begin creating milestones.
//                         </p>
//                     )}
//                 </div>
//             </div>
