...
const StatusBar = ({ leadId, leadData, isLost, isWon, statusRemarks, customDataFromParent = [] }) => {
  ...
  // Combine all remarks for timeline display
  const getAllRemarks = () => {
    // Combine regular status remarks with custom status data
    const allRemarks = [...(statusRemarks || []), ...customStatusData];
    
    // Filter out invalid remarks
    const validRemarks = allRemarks.filter(remark => 
      remark && 
      (remark.lead_status_remarks?.trim() || 
       remark.comments?.trim() || 
       remark.quotation_no || 
       remark.order_status || 
    ...
    return uniqueRemarks.sort((a, b) => 
      new Date(b.dcreated_dt || b.createdAt) - new Date(a.dcreated_dt || a.createdAt)
    );
  };
              ...
              return (
                <React.Fragment key={stage.id}>
                  <div className="flex flex-col items-center flex-1">
                    ...
                    <span className={`mt-2 text-sm text-center max-w-[100px] break-words ${ !isStageActive(stage) ? 'text-gray-400' : '' }`} >
                      {stage.name}
                      {!isStageActive(stage) && (
                ...
                case "quotation":
                  return (
                    <QuotationForm 
                      ...
                      onClose={() => { setOpenDialog(false); const newIndex = stages.findIndex(s => s.id === remarkStageId); updateStage(newIndex, remarkStageId); }}
                    />
                  );
                case "order":
                  return (
          ...
          <DialogContent>
            ...
            <div className="mt-4 mb-2">
              ...
              <select
                ...
                value={remarkData.assignedTo} onChange={handleAssignedToChange} disabled={remarkData.assignToMe}
              >
              ...
              <p className="text-xs text-gray-500 mt-1"> Optionally assign this lead to someone when submitting the remark </p>
            </div>
          ...
          >
            {selectedRemark && (
              <>
                ...
                {/* Show details based on status type */}
                {selectedRemark.status_name?.toLowerCase().includes('quotation') ? (
                  <div className="w-full space-y-3 mb-6">
                    <p className="text-sm text-gray-700">
                      <strong>Quotation No.:</strong> {selectedRemark.quotation_no || '-'}
                    </p>
                    <p className="text-sm text-gray-700">
                      <strong>Quotation Value:</strong> ₹{selectedRemark.quotation_value || '0'}
                    </p>
                    <p className="text-sm text-gray-700">
                      <strong>Quotation Date:</strong> {selectedRemark.quotation_date ? formatDateOnly(selectedRemark.quotation_date) : '-'}
                    </p>
                    {selectedRemark.comments && (
                      <p className="text-sm text-gray-700 break-words">
                        <strong>Comments:</strong> {selectedRemark.comments}
                      </p>
                    )}
                  </div>
                ) : selectedRemark.status_name?.toLowerCase().includes('order') ? (
                  <div className="w-full space-y-3 mb-6">
                    <p className="text-sm text-gray-700">
                ...
                ) : selectedRemark.status_name?.toLowerCase().includes('payment') ? (
                  <div className="w-full space-y-3 mb-6">
                    <p className="text-sm text-gray-700">
                    ...
                    {selectedRemark.next_payment_date && (
                      <p className="text-sm text-gray-700">
                        <strong>Next Payment Date:</strong> {formatDateOnly(selectedRemark.next_payment_date)}
                      </p>
                    )}
                    {selectedRemark.comments && (
                      <p className="text-sm text-gray-700 break-words">
                ...
                ) : selectedRemark.status_name?.toLowerCase().includes('billing') ? (
                  <div className="w-full space-y-3 mb-6">
                    <p className="text-sm text-gray-700">
                      <strong>Invoice Number:</strong> {selectedRemark.invoice_no || '-'}
                    </p>
                    {selectedRemark.invoice_date && (
                      <p className="text-sm text-gray-700">
                        <strong>Invoice Date:</strong> {formatDateOnly(selectedRemark.invoice_date)}
                      </p>
                    )}
                    {selectedRemark.comments && (
                      <p className="text-sm text-gray-700 break-words">
...
//         style={{ minWidth: "250px", maxWidth: "250px" }}  onClick={() => setSelectedRemark(remark)}
//       >
//         <div className="space-y-2 text-sm">
//           {/* QUOTATION STATUS */}
...
//                   <strong>Note:</strong> {remark.comments}
//                 </p>
//                )} 
//             </>
//           )
...
//             <>
//               <p className="text-sm font-medium text-gray-800">
//                 <strong>Invoice #:</strong> {remark.invoice_no || '-'}
//               </p>
//               {remark.invoice_date && (
//                 <p className="text-sm text-gray-700">
//                   <strong>Date:</strong> {formatDateOnly(remark.invoice_date)}
//                 </p>
//               )}
//               {remark.comments && (
//                 <p className="text-sm text-gray-700 line-clamp-2">
...
//               <strong>Remark:</strong> {remark.lead_status_remarks || remark.comments}
//             </p>
//           )}
