...
const apiEndPoint = import.meta.env.VITE_API_URL;
  ...
  setForm((prev) => {
    const exists = prev.iservice_id.includes(service.serviceId);

    return {
      ...prev,
      iservice_id: exists
        ? prev.iservice_id.filter(id => id !== service.serviceId)
        : [...prev.iservice_id, service.serviceId]
    };
  });
};
...
form.iservice_id.includes(sub.iservice_parent)
 &&
  ...
  const validateField = (name, value) => {
    ...
    if (name === "iservice_id" && (!Array.isArray(value) || value.length === 0)) {
  error = "Mandatory";
}
...
{filterService.map((item) => {
  const selected = form.iservice_id.includes(item.serviceId);
...
const handleSubmit = async (e) => {
  ...
  const basicLeadFields = [
    ...
    { label: "Organization Name", name: "corganization", required: false },
    { label: "Site Location", name: "cwebsite", required: false },
  ];
