...
export default function LeadsTable({ data }) {
  ...
  const handleLeadClick = (leadId) => { navigate(`/leaddetailview/${leadId}`);};
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {currentLeads.length > 0 ? (
              currentLeads.map((lead) => (
                <tr key={lead.id} className="hover:bg-gray-100 cursor-pointer transition-colors duration-200" onClick={() => handleLeadClick(lead.id)} >
