...
const ProfileHeader = () => {
  ...
  // Derive businessType from context
  // Business Type 1 = B2B (LeadForm), Business Type 2 = B2C (LeadFormB2C)
  const businessTypeId = useMemo(() => {
    return company?.result?.businessType?.id || company?.result?.ibusiness_type || null;
  }, [company]);
  ...
  const handleLeadFormOpen = () => {
    if (businessTypeId === 1 || businessTypeId === 2) {
  ...
  const handleGeneratePoster = () => { navigate("/generate-poster"); setShowAppMenu(false); };
  const handleMaps = () => { navigate("/maps"); setShowAppMenu(false); };
  const handleGSTVerification = () => { navigate("/gst-compliance"); setShowAppMenu(false); };
  const handleBulkMail = () => { navigate("/mailsender"); setShowAppMenu(false); };
  ...
  return (
    <div className="flex justify-end items-center gap-4 mb-6 relative">
      {/* Lead Form Modal using businessTypeId from Context */}
      {showLeadForm && (
        <div className="fixed inset-0 z-40 bg-black bg-opacity-30 flex justify-center items-center">
          <div className="bg-white p-6 rounded-3xl shadow-2xl w-11/12 md:w-3/4 max-h-[80vh] overflow-y-auto transition-all duration-300">
            {businessTypeId === 1 && <LeadForm onClose={handleLeadFormClose} />}
            {businessTypeId === 2 && <LeadFormB2C onClose={handleLeadFormClose} />}
          </div>
        </div>
      )}
      ...
      <button
        ...
        disabled={!(businessTypeId === 1 || businessTypeId === 2)}
        className={`px-5 py-2 rounded-full text-white font-medium 
          bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 
          border border-blue-300 shadow-lg 
          hover:from-blue-600 hover:to-blue-800 
          transition duration-300
          ${!(businessTypeId === 1 || businessTypeId === 2) ? "opacity-50 cursor-not-allowed" : "animate-pulse"}`}
      >
        + Create Lead
      </button>
      ...
      <div className="relative" ref={notificationRef}>
        <Bell onClick={handleOpenNotifications} className="w-10 h-10 border border-grey-600 rounded-full p-2 text-blue-600 cursor-pointer bg-white shadow-md hover:bg-blue-50 transition"
        />
        ...
        {showAppMenu && (
          <div className="absolute right-0 mt-2 w-60 bg-white border border-indigo-200 rounded-2xl shadow-2xl p-4 z-50 grid grid-cols-2 gap-3">
            {hasPdfPermission && pdfPermissions.map((item, index) => (
              <div key={index} onClick={handleGeneratePoster} className="flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-3 cursor-pointer transition">
            ...
            <div onClick={handleGSTVerification} className="flex flex-col items-center justify-center bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl p-3 cursor-pointer">
              <img src="/illustrations/courthouse.png" alt="gst" className="w-8 h-8 mb-1 opacity-90" />
              <span className="text-sm font-semibold text-green-700">GST</span>
            </div>

            <div onClick={handleMaps} className="flex flex-col items-center justify-center bg-gradient-to-br from-red-50 to-red-100 border border-red-200 rounded-xl p-3 cursor-pointer">
              <img src="/illustrations/map_dist.png" alt="maps" className="w-8 h-8 mb-1 opacity-90" />
              <span className="text-sm font-semibold text-red-700">Maps</span>
            </div>

            <div onClick={handleBulkMail} className="flex flex-col items-center justify-center bg-gradient-to-br from-yellow-50 to-yellow-100 border border-yellow-200 rounded-xl p-3 cursor-pointer">
              <img src="/illustrations/bulkMail.png" alt="mail" className="w-15 h-8 mb-1 opacity-90" />
              <span className="text-sm font-semibold text-yellow-700">Mail</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="relative" ref={dropdownRef}>
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setShowDropdown((prev) => !prev)}>
          <div className="w-12 h-12 rounded-full bg-green-600 flex items-center justify-center text-white font-semibold shadow-md">
            {profile.name ? profile.name.split(" ").map((n) => n[0]).slice(0, 2).join("").toUpperCase() : "UU"}
          </div>
        </div>

        {showDropdown && (
          <div className="absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-2xl border border-gray-200 p-5 text-sm z-50">
            <div className="flex justify-between items-start mb-2">
              <div>
                <div className="font-semibold text-gray-900">{profile.name}</div>
                <div className="text-gray-500">{profile.email}</div>
                <div className="text-gray-400 text-[10px] mt-1">Type ID: {businessTypeId}</div>
              </div>
              <X className="w-5 h-5 cursor-pointer text-gray-400" onClick={() => setShowDropdown(false)} />
            </div>
            <button onClick={handleLogout} className="w-full text-center bg-red-500 text-white font-semibold py-2 rounded-xl">Logout</button>
          </div>
        )}
      </div>
    </div>
