...
// Token is already retrieved at the top
...
const ReminderForm = () => {
  ...
  const [loggedInUserName, setLoggedInUserName] = useState(""); // New state for logged-in user's full name
  ...
  // --- Start: Added useEffect to parse user details from localStorage ---
  useEffect(() => {
    // Attempt to retrieve the 'user' string from localStorage
    const userString = localStorage.getItem("user");
    if (userString) {
      try {
        // Parse the JSON string into a JavaScript object
        const userObject = JSON.parse(userString);
        // Set the logged-in user's ID and full name if available
  ...
  }, []); // Run once on component mount to get user data
  // --- End: Added useEffect to parse user details from localStorage ---

  // Existing useEffect for speech recognition
  useEffect(() => {
  ...
  const handleChange = (e) => {
    ...
    // Fix: prevForm was undefined, should be prev
    setForm((prev) => ({
      ...prev,
  ...
  const handleAssignToMeChange = (e) => {
    ...
    if (checked) {
      // If "Assign to me" is checked, set 'assignt_to' to the logged-in user's ID
      setForm((prev) => ({
        ...prev,
        assignt_to: loggedInUserId,
      }));
    } else {
      // If unchecked, clear the 'assignt_to' field
      setForm((prev) => ({
        ...prev,
        assignt_to: "",
      }));
  ...
  const handleSubmit = async (e, status) => {
    ...
    // Ensure assignt_to is a number, as it comes from select value or loggedInUserId
    ...
    try {
      ...
      setAssignToMe(false); // Reset assign to me checkbox
  ...
  const getReminder = async () => { // Removed 'User' parameter as it's not used
    try {
  ...
  const fetchUsers = async () => {
    try {
      ...
      setUsers(res.data);
      // Removed the redundant logic here as loggedInUserId is now set from localStorage.
      // The original code was also looking for "Your Logged In User Name" which was a placeholder.
    } catch (err) {
