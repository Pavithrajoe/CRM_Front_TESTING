...
useEffect(() => {
    ...
    try {
      ...
      if (assignedUser && profile) {
        const mailPayload = {
          ...
          assignedTo: assignedUser?.cUser_name ?? assignedUser?.cFull_name ?? "",
  ...
  const renderWebsite = (url) => {
    ...
    try {
      const safeUrl = typeof url === "string" && url.trim() !== "" ? (url.startsWith("http") ? url : `https://${url}`) : null;
      if (!safeUrl) return <span>-</span>;
      return (
        <a href={safeUrl} target="_blank" rel="noopener noreferrer" className="text-blue-900 hover:underline w-[180px] break-words text-grey-900">
          {url}
        </a>
      );
    } catch (err) {
              ...
              <div className="p-4 bg-green-50 border border-green-200 rounded-xl shadow-sm mt-5 text-sm text-green-800 flex flex-col gap-3">
                {profile?.website_lead === true && (
                  <div className="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-600 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M4.083 9.922A6.955 6.955 0 0010 16a6.955 6.955 0 005.917-6.078 2.5 2.5 0 010-4.844A6.955 6.955 0 0010 4a6.955 6.955 0 00-5.917 6.078 2.5 2.5 0 010 4.844zM10 18a8 8 0 100-16 8 8 0 000 16zm-7.5-8a7.5 7.5 0 1015 0 7.5 7.5 0 00-15 0zM10 7a3 3 0 100 6 3 3 0 000-6z" clipRule="evenodd" />
                    </svg>
                ...
                {profile?.remarks && (
                  <div className="flex items-start gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-600 flex-shrink-0 mt-1" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-9-4a1 1 0 112 0v4a1 1 0 01-2 0V6zm1 8a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" clipRule="evenodd" />
                    </svg>
            ...
            <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl shadow-sm mt-5 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-5">
                ...
                <div className="relative w-full sm:w-64">
                  <select
                    id="assignUser"
                    className={`appearance-none w-full border border-gray-300 rounded-lg px-4 py-2 pr-10 text-sm text-gray-800 bg-white shadow-inner focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition-all duration-200 ${profile?.bactive === false ? "bg-gray-100 cursor-not-allowed" : ""}`}
                    onChange={(e) => {
            ...
            {latestAssignments.length === 0 ? (
              ...
              <div className="space-y-3">
                {latestAssignments.map((assignment) => (
                  <div key={assignment?.iassigned_id ?? Math.random()} className="text-sm text-gray-800 bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                    <p>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      ...
                      {assignment?.dcreate_dt ? new Date(assignment.dcreate_dt).toLocaleString() : "-"}
                    </p>
                  </div>
                ))}
              </div>
          ...
          <div className="mt-5 space-y-3">
            ...
            {Array.isArray(attachments) &&
              attachments.map((file) => {
                const filePath = file?.cattechment_path ?? file?.attachment_path ?? file?.path ?? null;
                const filename = typeof filePath === "string" ? filePath.split("/").pop() : file?.file_name ?? "attachment";

                return (
                  <div key={file?.ilead_id ?? file?.id ?? filename} className="text-sm text-gray-800 bg-white border border-gray-200 rounded-lg p-3 shadow-sm flex justify-between items-center">
                    <span className="font-medium truncate max-w-[70%] sm:max-w-[80%]">{filename}</span>
        ...
        <Dialog open={isAttachmentModalOpen} onClose={() => setIsAttachmentModalOpen(false)} className="relative z-50">
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <Dialog.Panel className="w-full max-w-sm sm:max-w-md bg-white p-6 rounded-2xl shadow-lg">
              ...
              <div
                {...getRootProps()}
                className={`border-2 border-dashed rounded-xl p-6 sm:p-8 text-center cursor-pointer transition-all duration-200 ${isDragActive ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:border-gray-400 bg-gray-50"}`}
              >
                <input {...getInputProps()} />
                {selectedFile ? (
                  ...
                  <>
                    <p className="text-sm text-gray-500">
                      Drag & drop a file here, or <span className="text-blue-900 font-medium">click to select</span>
                    </p>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  ...
                  className={`px-5 py-2 text-sm font-semibold rounded-lg text-white shadow-md ${uploading ? "bg-gray-400 cursor-not-allowed" : "bg-blue-900 hover:bg-blue-600"} transition-colors`}
                >
